...
function formatActivity(
  ...
  switch (entry.action) {
    ...
    case "redmine_linked": {
      ...
      if (fromId)
        return `changed linked Redmine issue from #${fromId} to ${label}`;
    ...
    case "time_logged": {
      ...
      let msg = `logged ${mins < 60 ? `${mins}m` : `${Math.floor(mins / 60)}h${mins % 60 ? ` ${mins % 60}m` : ""}`}`;
      if (activity) msg += ` (${activity})`;
          ...
          <div>
            ...
            <div className="mt-4 flex flex-col gap-3">
              {(() => {
                ...
                const groups: {
                  type: "activities" | "comment" | "time_entry";
                ...
                for (const entry of coalesced) {
                  ...
                  } else if (entry.type === "time_entry") {
                    groups.push({ type: "time_entry", entries: [entry] });
                  } else {
                ...
                return groups.map((group) => {
                  if (group.type === "time_entry") {
                    ...
                    return (
                      <div
                        key={entry.id}
                        className="px-4 flex items-start gap-2 text-xs text-muted-foreground"
                      >
                        <ActorAvatar actorType={entry.actor_type} actorId={entry.actor_id} size={18} />
                        <div className="flex-1 min-w-0">
                          <span className="font-medium text-foreground">
                            {entry.actor_name ?? (entry.actor_type === "agent" ? "Agent" : "User")}
                          </span>
                          {" logged "}
                          <span className="font-medium text-foreground">
                            {entry.duration_minutes != null ? fmtMins(entry.duration_minutes) : "time"}
                          </span>
                          {entry.activity_name && (
                            <span className="text-muted-foreground"> ({entry.activity_name})</span>
                          )}
                          {entry.time_entry_comment && (
                            <span className="italic"> — {entry.time_entry_comment}</span>
                          )}
                          {entry.actor_type === "agent" && entry.agent_task_id && (
                            <span className="ml-1 text-[10px] text-muted-foreground/60" title="Generated by Multica agent">
                              · via agent
                            </span>
                          )}
                        </div>
                        <span className="shrink-0 text-[10px]">{timeAgo(entry.created_at)}</span>
                      </div>
                    );
                  }
