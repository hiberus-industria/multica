                ...
                render={
                    <div
                        className={cn(
                            ...
                            (day.status === "complete" || day.status === "over" || day.status === "partial" || day.status === "missing") && "border-border",
...
export function MonthlyCalendarView({ year, month }: { year: number; month: number }) {
    ...
    const daysGrid = useMemo(() => {
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            const status = computeDayStatus(loggedMinutes, requiredHours, dayType, isFuture);
    ...
    const kpis = useMemo(() => {
        ...
        const totalRequired = pastWorkDays.reduce((sum, d) => sum + d.requiredHours * 60, 0);
        ...
        const completionPct = totalRequired > 0 ? Math.round((totalLogged / totalRequired) * 100) : 0;
