...
export function useTypeLabels(): Record<InboxItemType, string> {
  ...
  return {
    ...
    time_not_logged: t(($) => $.types.time_not_logged),
  };
}
...
export function InboxDetailLabel({ item }: { item: InboxItem }) {
  ...
  switch (item.type) {
    ...
    case "reaction_added": {
      ...
      if (emoji)
        return <span>{t(($) => $.labels.reacted_to_comment, { emoji })}</span>;
      return <span>{typeLabels[item.type]}</span>;
    }
    case "quick_create_done": {
      ...
      if (identifier)
        return (
          <span>{t(($) => $.labels.created_with_agent, { identifier })}</span>
        );
      return <span>{typeLabels[item.type]}</span>;
    }
    case "quick_create_failed": {
      ...
      if (detail)
        return <span>{t(($) => $.labels.failed_with_detail, { detail })}</span>;
      return <span>{typeLabels[item.type]}</span>;
    }
    default:
